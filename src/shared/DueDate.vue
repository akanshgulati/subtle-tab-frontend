<template>
    <div class="flex flex-center pointer font-xsmall">
        ...
        <transition mode="out-in">
                <span key='static' class="ml-10" v-if="!isEditing"
                      @click.stop="isEditing = true">Add due date</span>

            <input key='dynamic' type="date" v-else class="ml-10 due-date mb-0 border-0"/>
        </transition>
    </div>
</template>
<script>
